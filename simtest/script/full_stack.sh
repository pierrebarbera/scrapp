...
echo "run,seq_length,mut_rate,species,sample_size,pop_size,prune_fract,scrapp_mode,krd,norm_krd,norm_norm_krd,norm_unit_krd,abs_err_sum,abs_err_mean,abs_err_med,rel_err_sum,rel_err_mean,rel_err_med,norm_err_sum,norm_err_mean,norm_err_med" > results.csv
          ...
          for sample_size in 50; do
            for scrapp_mode in rootings bootstrap outgroup; do
              for i in {1..10}; do
                ...
                SCRAPP_SIM_CURDIR=${BASE}/runs/simulated/seq_length_${seq_length}/prune_fract_${prune_fract}/pop_size_${pop_size}/species_${species}/mut_rate_${mut_rate}/sample_size_${sample_size}/scrapp_mode_${scrapp_mode}/iter_${i}
                export SCRAPP_SIM_CURDIR
                rm -rf ${SCRAPP_SIM_CURDIR}/* 2> /dev/null

                printf "${run},${seq_length},${mut_rate},${species},${sample_size},${pop_size},${prune_fract},${scrapp_mode}," >> results.csv
