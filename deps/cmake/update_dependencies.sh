...
function get_commit_hash() {
    ...
    commit_hash=`git submodule status --recursive | grep "${libname} " | grep "${prefix}" | sed "s/.\([0-9a-f]*\) .*/\1/g" | tr -d "\n"`
...
update_commit_hash "mptp"
update_commit_hash "ParGenes"
update_commit_hash "MPIScheduler"
